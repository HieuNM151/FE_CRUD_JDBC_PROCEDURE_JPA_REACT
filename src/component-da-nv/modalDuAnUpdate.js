...
const ModalUpdateDA = ({ show, handleClose, idda , fetchDuAn }) => {
    ...
    const handleAddEmployee = () => {
        if (handleValidation()) {
            ...
            fetch(`http://localhost:8081/api/v1/du-an/update?id=${idda}`, {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        if (idda && idda.trim() !== "") {
            fetch(`http://localhost:8081/api/v1/du-an/details?id=${idda}`)
                .then((res) => res.json())
                .then((resp) => {
                    setTenduan(resp.tenduan);
                    setngaybatdau(resp.ngaybatdau);
                    setngayketthuc(resp.ngayketthuc);
                    settrangthai(resp.trangthai ? "Đang diễn ra" : "Kết thúc"); 
                })
                .catch((err) => {
